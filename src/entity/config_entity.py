...
class TrainingPipelineConfig:
    """Configuration for overall training pipeline."""

    def __init__(self):
        try:
            timestamp = datetime.now().strftime('%m%d%Y__%H%M%S')
            self.artifact_dir = os.path.join(os.getcwd(), "artifact", timestamp)
...
class DataValidationConfig:
    ...
    def __init__(self, training_pipeline_config: TrainingPipelineConfig):
        try:
            ...
            self.base_file_path = os.path.join("dataset", "HRDataset_v14.csv")
        except Exception as e:
            raise ProjectException(e, sys)
...
class ModelEvaluationConfig:
    ...
    def __init__(self, training_pipeline_config: TrainingPipelineConfig):
        try:
            ...
            self.report_path = os.path.join(self.model_evaluation_dir, "evaluation_report.yaml")
